...
def place_ships_randomly(playerGridY, gridSize, shipsLocationPlayer, shipsLocationBot, placePlayerRandom, gridY): #Randomly generates spots on the grid for the ship
    ...
    if placePlayerRandom == True:
        ...
        return  playerGridY, playerGridY, shipsLocationPlayer
...
def displayGrid(displayGridList, displayPlayerGrid): #Grid shown to player
    ...
    for i in range(0, length): #This prints for the length of display list
        ...
        Fore.RED
    if (i < 9):
        print(Fore.RED + "    A    B    C    D    E    F    G    H    I    J")
    else:
        print(Fore.RED +"     A    B    C    D    E    F    G    H    I    J")
    for i in range(0, length):
...
def user_shot(turn, displayGridList, shipsLocationBot, shipsHit,displayPlayerGrid, gridSize):#user inputs in coordinates 
    ...
    if turn == 1:
        ...
        while (userPlacingShot == 1):
            user_cord = str(input("Where do you want to shoot? Coordinates plz e.g A,3: ")+ "~") #Makes a string of user's input plus ~ at the end to let the program know that's the end of the string
            print (user_cord)
    ...
    return shipsHit, displayPlayerGrid, displayGridList
